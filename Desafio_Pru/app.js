...
const {middleLogger, middleLoggerWarm,middleLoggerError} = require('./middlewares/middlewLogger.js');
...
function conectarDB(){
    try {
        const connect =  mongoose.connect("mongodb://localhost:27019/UsersDB", {
        useNewUrlParser: true,
        useUnifiedTopology: true
    });
    } catch (error) {
        console.log(error);
        throw new Error('error en conectarDB')
    }
...
app.use('/',middleLogger, users);
//app.use( middleLoggerError);
